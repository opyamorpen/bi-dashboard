...
const S: any = {
  ...
  btn: (p: boolean) => ({ padding: '8px 20px', borderRadius: 4, border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: 500, background: p ? '#1677ff' : '#f0f0f0', color: p ? '#fff' : '#333' }),
...
export const AddCardModal: React.FC<Props> = ({ datasets: propDatasets, onAdd, onCancel }) => {
  ...
  const [datasetUuid, setDatasetUuid] = useState('default_issue_dataset')
  ...
  useEffect(() => {
    if (datasets.length === 0) {
      apiGet('/bi/datasets').then((res: any) => {
        const list = res.data || []
        setDatasets(list)
  ...
  useEffect(() => {
    ...
    apiGet(`/bi/dataset/${datasetUuid}`).then((res: any) => {
    ...
    <div style={S.modalOverlay} onClick={onCancel}>
      <div style={S.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={S.section}>
          ...
          <select style={S.select} value={datasetUuid} onChange={(e) => setDatasetUuid(e.target.value)}>
            {datasets.map((d: any) => <option key={d.dataset_uuid} value={d.dataset_uuid}>{d.name}</option>)}
          </select>
        </div>
        {chartType !== 'number' && (
          <div style={S.section}>
            <label style={S.label}>分组维度</label>
            <select style={S.select} value={selectedDim} onChange={(e) => setSelectedDim(e.target.value)}>
              <option value="">无</option>
              {dimFields.map((f: any) => <option key={f.key} value={f.key}>{f.label}</option>)}
            </select>
          </div>
        )}
        <div style={S.section}>
        ...
        <div style={{ marginTop: 24, display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          ...
          <button style={S.btn(true)} onClick={handleAdd}>添加</button>
        </div>
      </div>
    </div>
  )
