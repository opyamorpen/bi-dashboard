...
const S: any = {
  ...
  msg: (ok: boolean) => ({ fontSize: 13, padding: '8px 12px', borderRadius: 4, marginBottom: 16, background: ok ? '#f6ffed' : '#fff2f0', color: ok ? '#52c41a' : '#ff4d4f', border: `1px solid ${ok ? '#b7eb8f' : '#ffccc7'}` }),
}

const ConfigApp: React.FC = () => {
  const [config, setConfig] = useState<any>({ base_url: '', model: '', supports_vision: true, api_key: '' })
      ...
      {loading ? (
        ...
        <div style={S.form}>
          <div style={S.section}>
            ...
            <input style={S.input} value={config.base_url || ''} onChange={(e) => setConfig({ ...config, base_url: e.target.value })} placeholder="https://api.openai.com/v1" />
            <div style={S.help}>支持 OpenAI 兼容的 /chat/completions 服务地址。</div>
          </div>
          <div style={S.section}>
            <label style={S.label}>Model</label>
            <input style={S.input} value={config.model || ''} onChange={(e) => setConfig({ ...config, model: e.target.value })} placeholder="gpt-4.1" />
          </div>
          <div style={S.section}>
            <label style={S.label}>API Key</label>
            <input style={S.input} type="password" value={config.api_key || ''} onChange={(e) => setConfig({ ...config, api_key: e.target.value })} placeholder={config.has_api_key ? '已配置，留空则不修改' : '请输入 API Key'} />
            <div style={S.help}>保存后不会在前端回显明文密钥。</div>
          </div>
          <div style={S.section}>
            <label style={S.row}>
              <input type="checkbox" checked={Boolean(config.supports_vision)} onChange={(e) => setConfig({ ...config, supports_vision: e.target.checked })} />
              支持图片输入
            </label>
          </div>
          <button style={S.btn(true)} onClick={save} disabled={saving}>{saving ? '保存中...' : '保存配置'}</button>
        </div>
      )}
    </div>
  )
